...
##### plot allele frequency in chrA01-10
```{r}
...
##### plot allele frequency in chrC01-09
```{r}
...
### Try filtering for F2 good SNPs (extract only SNPs which match the F2 good SNPs)

```{r message=FALSE}
F2snps <- read_tsv("Final_F2_SNP_Sites.tab") #18,226 SNPs
```

```{r message=FALSE}
data2.F2.filtered <- semi_join(data2,F2snps)

dim(data2) #our numbers of SNPs
dim(F2snps)#number of F2 SNPs
dim(data2.F2.filtered) #number of SNPs that match the F2 SNPs(-> too few!)
```
...
data3.F2 #1,997 SNPs filtered by F2
```


##### plot abs diff of allele freq along entire genome
```{r}
  data3.F2 %>% filter(!grepl("random", CHROM)) %>%
    ggplot(aes(x=POS,y=abs_delta_af_10,group=CHROM,color=CHROM)) +
    geom_point() +
    geom_line() +
    ggtitle("absolute diff. of allele freq. between Rf and MS pool_filtered by F2 SNPs") +
    ylim(0,1)
  ggsave("Delta_af_F2_filter_entire_genome.png", width = 8, height = 12, device = "png")
```
pick chrC09!

##### plot abs diff of allele freq bt chromosome
```{r}
for (chr in chrom.names) {
...
### Try filtering for 505 collection SNPs(extract only SNPs which match the 505 collection SNPs)
...
dim(data2) #number of SNPs in our data set
dim(snps.505) #number of SNPs in 505 collection
dim(data2.505.filtered) #number of SNPs that match the SNPs in 505 collection
```
...
```{r}
  data3.505 %>% filter(!grepl("random", CHROM)) %>%
    ...
    geom_point() +
    geom_line() +
    ggtitle("absolute diff. of allele freq. between Rf and MS pool_filtered by 505 SNPs") +
    ylim(0,1)
...
### Try for filtering Illumina SNPs
```{r}
data.illumina <- read_tsv("illumina.filtered.vcf.recode.vcf", comment="#", col_names = cnames)
data.illumina
SNP.illumina <- data.illumina %>% select(CHROM, POS)
SNP.illumina # 97,877 SNPs
```

```{r}
data2.illumina.filtered <- semi_join(data2,SNP.illumina) #return all rows from data2 where there are matching values in snp.illumina

dim(data2) #number of SNPs in our data set
dim(data.illumina) #number of SNPs in illumina
dim(data2.illumina.filtered)
```
```{r}
data3.illumina <- data2.illumina.filtered %>%
  group_by(CHROM) %>%
  mutate(abs_delta_af = abs(S95_AO/S95_DP - S96_AO/S96_DP),
         abs_delta_af_10=roll_mean(abs_delta_af,n=10,fill=NaN)
         )
...
##### plot abs diff of allele freq along entire genome
```{r}
 data3.illumina %>% filter(!grepl("random", CHROM)) %>%
    ggplot(aes(x=POS,y=abs_delta_af_10,group=CHROM,color=CHROM)) +
    geom_point() +
    geom_line() +
    ggtitle("absolute diff. of allele freq. between Rf and MS pool_filtered by illumina SNPS)") +
    ylim(0,1)
  ggsave("Delta_af_illumina_filter_entire_genome.png", width = 16, height = 20, device = "png")
```
There seems no finding which stands out..

##### abs diff of allele freq by chromosome
```{r}
for (chr in chrom.names) {
  data3.illumina %>% filter(str_detect(CHROM,chr)) %>%
    ggplot(aes(x=POS,y=abs_delta_af_10)) +
    geom_point(aes(y=abs_delta_af),shape=1) +
    geom_line(color="blue") +
    ggtitle(chr) +
    ylim(0,1)
  ggsave(str_c("Delta_af_illumina_filter",chr,".png"),height = 8, width = 12)
}
```
...
```{r}
region1.rd <- RangedData(IRanges(start=4.35e07,end=4.38e07,names="Region1"),space="chrC09") 
region1.rd 

overlaps1_4.35_4.38 <- as.data.frame(as.matrix(findOverlaps(ranges(region1.rd),ranges(genes.rd))))
nrow(overlaps1_4.35_4.38) 

overlaps1_4.35_4.38$locus <- genes$locus[overlaps1_4.35_4.38 $subjectHits]

write.csv(overlaps1_4.35_4.38,"overlaps1_4.35_4.38.csv")
```
...
in the interval 4E7-5E7 on chrC09
```{r}
overlaps1_4.35_4.38 <- read.csv("overlaps1_4.35_4.38.csv") 
a <- overlaps1_4.35_4.38 %>%
  mutate(napus_ID=locus) %>% select(subjectHits, napus_ID)  
overlaps1_4.35_4.38_match_ara <- semi_join(napus_vs_ara, a, by="napus_ID")
overlaps1_4.35_4.38_match_ara 
```

in the interval 4.35E7-4.38E7 on chrC09
```{r}
overlaps1_4.35_4.38 <- read.csv("overlaps1_4.35_4.38.csv") #53 hits
...
left_join(b, napus_vs_ara)
```


in the interval 4.370E7-4.373E7 on chrC09
```{r}
overlaps1_4.370_4.373 <- read.csv("overlaps1_4.370_4.373.csv") #6 hits
